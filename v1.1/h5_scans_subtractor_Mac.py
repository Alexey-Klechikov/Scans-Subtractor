...
class Ui_MainWindow(QtGui.QMainWindow):
    def setupUi(self, MainWindow):
        ...
        MainWindow.resize(665, 528)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(MainWindow.sizePolicy().hasHeightForWidth())
        MainWindow.setSizePolicy(sizePolicy)
        MainWindow.setMinimumSize(QtCore.QSize(665, 528))
        MainWindow.setMaximumSize(QtCore.QSize(665, 528))
        ...
        self.checkBox_dev_by_monitor.setText("Devide scans by monitors before subtraction ")
    ...
    def draw_det_A_frame(self):
        ...
        with h5py.File(self.lineEdit_name_scan_A.text(), 'r') as file:
            ...
            detector_image_A = file[list(file.keys())[0]].get("instrument").get('detectors').get(scan_psd_A).get('data')[int(self.comboBox_point_number_A.currentText())]
            detector_image_A = numpy.around(detector_image_A, decimals=0).astype(int)
            detector_image_A = numpy.subtract(detector_image_A, numpy.zeros((detector_image_A.shape[0], detector_image_A.shape[1])))

            self.graphicsView_scan_A.setImage(detector_image_A, axes={'x':1, 'y':0}, levels=(0,0.1))
    ...
    def draw_det_B_frame(self):
        ...
        with h5py.File(self.lineEdit_name_scan_B.text(), 'r') as file:
            ...
            detector_image_B = file[list(file.keys())[0]].get("instrument").get('detectors').get(scan_psd_B).get('data')[int(self.comboBox_point_number_B.currentText())]
            detector_image_B = numpy.around(detector_image_B, decimals=0).astype(int)
            detector_image_B = numpy.subtract(detector_image_B, numpy.zeros((detector_image_B.shape[0], detector_image_B.shape[1])))

            self.graphicsView_scan_B.setImage(detector_image_B, axes={'x':1, 'y':0}, levels=(0,0.1))
    ...
    def draw_diff(self):
        ...
        if not self.lineEdit_difference.text().find("A(") == -1:
            ...
            with h5py.File(self.lineEdit_name_scan_A.text(), 'r') as file:
                detector_image_A = file[list(file.keys())[0]].get("instrument").get('detectors').get(scan_psd_A).get('data')[index_psd_A]
                detector_image_A = numpy.around(detector_image_A, decimals=0).astype(int)
                self.detector_image_A = numpy.subtract(detector_image_A, numpy.zeros((detector_image_A.shape[0], detector_image_A.shape[1])))
        ...
        if not self.lineEdit_difference.text().find("B(") == -1:
            ...
            with h5py.File(self.lineEdit_name_scan_B.text(), 'r') as file:
                detector_image_B = file[list(file.keys())[0]].get("instrument").get('detectors').get(scan_psd_B).get('data')[index_psd_B]
                detector_image_B = numpy.around(detector_image_B, decimals=0).astype(int)
                self.detector_image_B = numpy.subtract(detector_image_B, numpy.zeros((detector_image_B.shape[0], detector_image_B.shape[1])))
        ...
        self.statusbar.showMessage('Subtraction "' + self.lineEdit_difference.text() + '" is performed.')
