...
class Ui_MainWindow(QtGui.QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        MainWindow.setMinimumSize(QtCore.QSize(1100, 807))
        MainWindow.setMaximumSize(QtCore.QSize(1100, 807))
    ...
    def draw_res(self):
        ...
        if not self.line_A == "":
            ...
            with h5py.File(self.lineEdit_name_scan_A.text(), 'r') as file:
                ...
                for index, scaler in enumerate(file[list(file.keys())[0]].get("instrument").get('scalers').get('SPEC_counter_mnemonics')):
                    ...
                    if monitor_A_identifier in str(scaler): monitor_A = numpy.array(file[list(file.keys())[0]].get("instrument").get('scalers').get('data')).T[index][int(self.comboBox_point_number_A.currentText())]
        ...
        if not self.line_B == "":
            ...
            with h5py.File(self.lineEdit_name_scan_B.text(), 'r') as file:
                ...
                # seems to be a bug in numpy arrays imported from hdf5 files. Need to redefine array type to int.
                detector_image_B = numpy.around(detector_image_B, decimals=0).astype(int)
                self.detector_image_B = numpy.subtract(detector_image_B, numpy.zeros(
                    (detector_image_B.shape[0], detector_image_B.shape[1])))
                ...
                for index, scaler in enumerate(file[list(file.keys())[0]].get("instrument").get('scalers').get('SPEC_counter_mnemonics')):
                    ...
                    if monitor_B_identifier in str(scaler): monitor_B =  numpy.array(file[list(file.keys())[0]].get("instrument").get('scalers').get('data')).T[index][int(self.comboBox_point_number_B.currentText())]
