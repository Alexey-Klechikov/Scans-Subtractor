'''
Install with:
* Windows - pyinstaller --onefile --noconsole -i"C:\icon.ico" --add-data C:\icon.ico;images C:\pySAsum_V1.4.1.py
* MacOS - pyinstaller --onefile --windowed pySAsum_V1.4.1.py
'''
...
class GUI(Ui_MainWindow):
    ...
    def f_buttons_roi(self):
        ...
        if self.sender().objectName() == "pushButton_result_ROI":
            ...
            if self.roi_show == 0:
                self.graphicsView_result.setGeometry(QtCore.QRect(3, 32+self.groupbox_os_displ, 770, 390))
                self.roi_show = 1
            else:
                self.graphicsView_result.setGeometry(QtCore.QRect(3, 32+self.groupbox_os_displ, 770, 684))
                self.roi_show = 0
